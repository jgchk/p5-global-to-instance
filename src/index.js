...
const ast2code = (ast, options = opts) =>
  escodegen.generate(ast, options.escodegen).code
const code2ast = (code, options = opts) =>
  esprima.parseModule(code, options.esprima)
...
function wrapP5Scope(ast) {
  ...
  for (const p5var of p5vars) {
    code = code.replace(
      new RegExp(`([^.\\w])(${p5var})[;\\s]?`, 'gm'),
      (match, p1, p2) => `${p1}${instance}.${p2}`
    )
  }
  for (const p5func of p5funcs) {
    code = code.replace(
      new RegExp(`([^.\\w])(${p5func}[(])`, 'gm'),
      (match, p1, p2) => `${p1}${instance}.${p2}`
    )
  }
